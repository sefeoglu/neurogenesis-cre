...
def main():
    ...
    for run_id in range(1,4):
        ...
        for task_id in range(1, 11):
            train_data = read_json(f"/content/tacred/final/train/run_1/task{task_id}/train_1.json")
            train_prepared = data_preparation(train_data)
            test_data = read_json(f"/content/tacred/final/test/run_1/task{task_id}/test_1.json")
            test_prepared = data_preparation(test_data)
            val_data = read_json(f"/content/tacred/final/train/run_1/task{task_id}/dev_1.json")
            ...
            model = BertForSequenceClassification_Neuro(config, pretrained_model_name='bert-large-uncased', num_classes=len(label_to_int), use_custom_encoder=True)
            # else:
            ...
            for i, epoch in enumerate(epoch_list):
                ...
                for batch in batch_list:
                    ...
                    write_json(train_hist, f"./neurogenesis_results_low/train_hist_{run_id}_{task_id}.json")
                    model_name_hf = f"bert_large_baseline_neurogenesis_{run_id}_{task_id}"
                    ...
                    write_json(results, f"./neurogenesis_results_low/results_{run_id}.json")
